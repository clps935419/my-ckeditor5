...
export default class ListStyleEditing extends Plugin {
    ...
    init() {
        ...
        editor.commands.add(
            'listStyle',
            new ListStyleCommand(editor, DEFAULT_LIST_TYPE)
        );
        ...
        this.listenTo(
            ...
            '_executeCleanup',
            fixListAfterIndentListCommand(editor)
        );
        this.listenTo(
            editor.commands.get('outdentList'),
            '_executeCleanup',
            fixListAfterOutdentListCommand(editor)
        );

        this.listenTo(
            editor.commands.get('bulletedList'),
            '_executeCleanup',
            restoreDefaultListStyle(editor)
        );
        this.listenTo(
        ...
        model.document.registerPostFixer(
            fixListStyleAttributeOnListItemElements(editor)
        );
    ...
    afterInit() {
        ...
        if (editor.commands.get('todoList')) {
            editor.model.document.registerPostFixer(
                removeListStyleAttributeFromTodoList(editor)
            );
        }
    }
    ...
    _mergeListStyleAttributeWhileMergingLists() {
        ...
        this.listenTo(
            ...
            (evt, [selection]) => {
                ...
                // <listItem listIndent="0" listType="bulleted" listStyle="square">UL List item 1</listItem>
                // <listItem listIndent="1" listType="bulleted" listStyle="square">UL List []item 1.1</listItem>
                const mostOuterItemList = getSiblingListItem(
                    firstPosition.parent,
                    {
                        sameIndent: true,
                ...
                if (
                    mostOuterItemList.getAttribute('listType') ===
                    nextSibling.getAttribute('listType')
                ) {
        ...
        this.listenTo(
            ...
            () => {
                ...
                model.change((writer) => {
                    ...
                    // <listItem listIndent="0" listType="bulleted" listStyle="circle">[]UL List item 1</listItem>
                    // <listItem listIndent="0" listType="bulleted" listStyle="circle">UL List item 2</listItem>
                    const secondListMostOuterItem = getSiblingListItem(
                        firstMostOuterItem.nextSibling,
                        {
                            sameIndent: true,
                            listIndent:
                                firstMostOuterItem.getAttribute('listIndent'),
                            direction: 'forward',
                        }
                    );
                    ...
                    const items = [
                        ...
                        ...getSiblingNodes(
                            writer.createPositionAt(secondListMostOuterItem, 0),
                            'forward'
                        ),
                    ];
                    ...
                    for (const listItem of items) {
                        writer.setAttribute(
                            'listStyle',
                            firstMostOuterItem.getAttribute('listStyle'),
                            listItem
                        );
                    }
                });
    ...
    return (dispatcher) => {
        dispatcher.on(
            ...
            (evt, data, conversionApi) => {
                ...
                const listStyle =
                    listParent.getStyle('list-style-type') || DEFAULT_LIST_TYPE;
                const listItem =
...
function downcastListStyleAttribute() {
    return (dispatcher) => {
        dispatcher.on(
            ...
            (evt, data, conversionApi) => {
                ...
                const previousElement = getSiblingListItem(
                ...
                const viewItem =
                    conversionApi.mapper.toViewElement(currentElement);
                // console.log(
                //     '---*---',
                //     currentElement,
                //     previousElement,
                ...
                if (!areRepresentingSameList(currentElement, previousElement)) {
                    viewWriter.breakContainer(
                        viewWriter.createPositionBefore(viewItem)
                    );
                }
                //把OL樣式設定取消掉
                // console.warn('設定')
    ...
    function areRepresentingSameList(listItem1, listItem2) {
        return (
            listItem2 &&
            listItem1.getAttribute('listType') ===
                listItem2.getAttribute('listType') &&
            listItem1.getAttribute('listIndent') ===
                listItem2.getAttribute('listIndent') &&
            listItem1.getAttribute('listStyle') ===
                listItem2.getAttribute('listStyle')
        );
    }
...
function fixListAfterIndentListCommand(editor) {
    return (evt, changedItems) => {
        ...
        const itemsToUpdate = changedItems.filter(
            (item) => item.getAttribute('listIndent') === rootIndent
        );
        ...
        if (
            root.previousSibling.getAttribute('listIndent') + 1 ===
            rootIndent
        ) {
...
function fixListAfterOutdentListCommand(editor) {
    return (evt, changedItems) => {
        ...
        editor.model.change((writer) => {
            const itemsToUpdate = changedItems.filter(
                (item) => item.getAttribute('listIndent') === indent
            );
...
function fixListStyleAttributeOnListItemElements(editor) {
    return (writer) => {
        let wasFixed = false;
        const insertedListItems = getChangedListItems(
        ...
        let existingListItem =
            insertedListItems[insertedListItems.length - 1].nextSibling;
        ...
        if (!existingListItem || !existingListItem.is('element', 'listItem')) {
            existingListItem =
                insertedListItems[insertedListItems.length - 1].previousSibling;
            ...
            if (existingListItem) {
                ...
                // ■ Paragraph[]  // <-- The inserted item.
                while (
                    existingListItem.is('element', 'listItem') &&
                    existingListItem.getAttribute('listIndent') !== indent
                ) {
        ...
        for (const item of insertedListItems) {
            if (!item.hasAttribute('listStyle')) {
                if (shouldInheritListType(existingListItem, item)) {
                    writer.setAttribute(
                        'listStyle',
                        existingListItem.getAttribute('listStyle'),
                        item
                    );
                } else {
                ...
                if (
                    shouldInheritListTypeFromPreviousItem(previousSibling, item)
                ) {
                    writer.setAttribute(
                        'listStyle',
                        previousSibling.getAttribute('listStyle'),
                        item
                    );
...
function shouldInheritListType(baseItem, itemToChange) {
    ...
    if (
        baseItem.getAttribute('listType') !==
        itemToChange.getAttribute('listType')
    ) {
        return false;
    }
...
function shouldInheritListTypeFromPreviousItem(previousItem, itemToChange) {
    ...
    if (
        itemToChange.getAttribute('listType') !==
        previousItem.getAttribute('listType')
    ) {
        return false;
    }
    ...
    if (
        previousItemIndent < 1 ||
        previousItemIndent !== itemToChange.getAttribute('listIndent')
    ) {
        return false;
    }
    ...
    if (
        !previousItemListStyle ||
        previousItemListStyle === itemToChange.getAttribute('listStyle')
    ) {
        return false;
    }
...
function removeListStyleAttributeFromTodoList(editor) {
    return (writer) => {
        const todoListItems = getChangedListItems(
            editor.model.document.differ.getChanges()
        ).filter((item) => {
...
function restoreDefaultListStyle(editor) {
    return (evt, changedItems) => {
        changedItems = changedItems.filter((item) =>
